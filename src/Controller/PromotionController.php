...
class PromotionController extends AbstractController
{
    ...
    public function index(PromotionRepository $promotionRepository, Request $request, PaginatorInterface $paginator): Response
    {
        $pagination = $paginator->paginate(
            $promotionRepository->findBy([], ["id" => "DESC"]), /* query NOT result */
    ...
    public function new(Request $request): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $promo = $form['photo']->getData();
            if($promo){
                $originalFilename = pathinfo($promo->getClientOriginalName(), PATHINFO_FILENAME);
                $safeFilename = transliterator_transliterate('Any-Latin; Latin-ASCII; [^A-Za-z0-9_] remove; Lower()', $originalFilename);
                $newFilename = $safeFilename.'-'.uniqid().'.'.$promo->guessExtension();
                $promo->move(
                    $this->getParameter('promotion'),
                    $newFilename
                );
                $promotion->setPhoto($newFilename);
                $promotion->setNom($originalFilename);
            }
